...
import { Lucid, Blockfrost, fromText, Data, getAddressDetails, applyParamsToScript } from "../lucid-cardano";
...
const utxosTontine = await lucid.utxosAt(getAdress());
...
async function tontineOuvre(addressTontine, numeroTontine) {
...
async function tontineFerme(addressTontine, numeroTontine) {
    const dtmClose = Data.to(datumTontineClose, TontineDatum);
    const dtmOpen = Data.to(datumTontineOpen, TontineDatum);
...
async function tontinePay(beneficiaire, numeroTontine) {
    const dtmClose = Data.to(datumTontineClose, TontineDatum);
